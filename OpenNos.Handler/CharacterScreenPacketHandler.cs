    ...
    public class CharacterScreenPacketHandler : IPacketHandler
    {
        ...
        public void LoadCharacters(string packet)
        {
            ...
            if (Session.Account == null)
            {
                ...
                try
                {
                    if (loginPacketParts.Length == 9
                        && loginPacketParts[4] == "DAC")
                    {
                        userName = loginPacketParts[5];
                        lastCharacter = ServerCommunicationClient.Instance.HubProxy.Invoke<long>("LastCharacter", userName).Result;
                        Session.LastPort = ServerCommunicationClient.Instance.HubProxy.Invoke<long>("LastPort", userName).Result;
                        bDAC = true;
                    }
                    else
                    {
                        if (loginPacketParts.Length > 4)
                            userName = loginPacketParts[4];
                    }

                    hasRegisteredAccountLogin = userName == null ? false : ServerCommunicationClient.Instance.HubProxy.Invoke<bool>("HasRegisteredAccountLogin", userName, Session.SessionId).Result;
                }
                catch (Exception ex)
                {
                ...
                {
                    AccountDTO accountDTO = DAOFactory.AccountDAO.LoadByName(userName);

                    if (accountDTO != null)
                    {
                        if (bDAC || accountDTO.Password.ToLower().Equals(EncryptionBase.Sha512(loginPacketParts[6])))
                        {
                            if (bDAC && lastCharacter == -1)
                            {
                                Logger.Log.ErrorFormat($"Client {Session.ClientId} forced Disconnection, invalid LastCharacter slot.");
                                Session.Disconnect();
                                return;
                            }
            ...
            if (bDAC)
            {
                SelectCharacter("0 select " + lastCharacter.ToString());
                return;
            }
            ...
            try
            {
                ...
                if (Session?.Account != null && !Session.HasSelectedCharacter)
                {
                    ...
                    if (character != null)
                    {
                        ...
                        if (System.Configuration.ConfigurationManager.AppSettings["Act4Mode"] == "n"
                           && Session.CurrentMapInstance.Map.MapTypes.Any(s => s.MapTypeId == (short)MapTypeEnum.Act4))
                        {
                            ...
                            Session.Character.MapX = 39;
                            Session.Character.MapY = 43;

                            NRunHandler.SwitchToA4Server(Session, false);
                            return;
                        }
