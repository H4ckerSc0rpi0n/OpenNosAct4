    ...
    public class BattlePacketHandler : IPacketHandler
    {
            ...
            if (Session.Character.CanFight && useSkillPacket != null)
            {
                ...
                switch (useSkillPacket.UserType)
                {
                    case UserType.Monster:
                        ...
                        {
                            MapMonster monster = Session.CurrentMapInstance.GetMonster(useSkillPacket.MapMonsterId);

                            if ((Session.Character.Faction == 1 && monster?.MonsterVNum == 679)
                                || (Session.Character.Faction == 2 && monster?.MonsterVNum == 680))
                            {
                                Session.SendPacket("cancel 2 0");
                                return;
                            }
        ...
        private void PVPHit(HitRequest hitRequest, ClientSession target)
        {
            if (target.CurrentMapInstance.Map.MapTypes.Any(s => s.MapTypeId == (short)MapTypeEnum.Act4)
                && target.Character.Faction == hitRequest.Session.Character.Faction)
            {
            ...
            if (target.Character.Hp > 0 && hitRequest.Session.Character.Hp > 0)
            {
                ...
                if (!IsAlive)
                {
                    if (target.CurrentMapInstance.Map.MapTypes.Any(s => s.MapTypeId == (short)MapTypeEnum.Act4))
                    {
                        ...
                        if (target.Character.Act4Points > 0)
                            target.Character.Act4Points -= 1;

                        if (target.Character.Level >= hitRequest.Session.Character.Level - 10
                                && target.Character.Level <= hitRequest.Session.Character.Level + 10)
                        {
                        ...
                        int _x = target.Character.Level * 50;

                        if (target.Character.Reput > _x)
                        {
                            target.Character.Reput -= (_x / 2);
                        }
                        else
                        {
                            target.Character.Reput = 0;
                        }

                        hitRequest.Session.Character.Reput += _x;
                        ...
                        target.SendPacket(target.Character.GenerateSay(string.Format(Language.Instance.GetMessageFromKey("LOSE_REP"), (short)(_x)), 11));
                        hitRequest.Session.SendPacket(target.Character.GenerateSay(string.Format(Language.Instance.GetMessageFromKey("WON_REP"), (short)(_x)), 12));
                        //}
                        ...
                        target.SendPacket("scr " + target.Character.Act4Kill + " " + target.Character.Act4Dead + " " + target.Character.Act4Points + " " + target.Character.Compliment + " 0 0");
                        hitRequest.Session.SendPacket("scr " + hitRequest.Session.Character.Act4Kill + " " + hitRequest.Session.Character.Act4Dead + " " + hitRequest.Session.Character.Act4Points + " " + hitRequest.Session.Character.Compliment + " 0 0");

                        target.CurrentMapInstance?.Broadcast(target, "c_mode 1 " + target.Character.CharacterId + " 1564 0 0 0", ReceiverType.All);
                        target.CurrentMapInstance?.Broadcast(target, "revive 1 " + target.Character.CharacterId + " 0", ReceiverType.All);
                        target.CurrentMapInstance?.Broadcast(target, "tp 1 " + target.Character.CharacterId + " " + target.Character.MapX + " " + target.Character.MapY + " 0", ReceiverType.All);

                        Observable.Timer(TimeSpan.FromMilliseconds(1500))
                            .Subscribe(
                            o =>
                            {
                                target.CurrentMapInstance?.Broadcast(target, "revive 1 " + target.Character.CharacterId + " 1", ReceiverType.All);
                            });
                        ...
                        Observable.Timer(TimeSpan.FromMilliseconds(30000))
                            ...
                            o =>
                            {
                                ServerManager.Instance.ReviveFirstPosition(target.Character.CharacterId);
                                ...
                                int mapId = target.Character.Faction == 1 ? 130 : 131;
                                ServerManager.Instance.ChangeMap(target.Character.CharacterId, (short)mapId, x, y);
