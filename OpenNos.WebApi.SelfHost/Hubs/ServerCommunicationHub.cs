...
namespace OpenNos.WebApi.SelfHost
{
    public class ServerCommunicationHub : Hub
    {
        ...
        public long LastCharacter(string accountName)
        {
            if (ServerCommunicationHelper.Instance.RegisteredAccountLogins.ContainsKey(accountName.ToLower()))
                return ServerCommunicationHelper.Instance.RegisteredAccountLogins[accountName.ToLower()][1];

            return -1;
        }

        public long LastPort(string accountName)
        {
            if (ServerCommunicationHelper.Instance.RegisteredAccountLogins.ContainsKey(accountName.ToLower()))
                return ServerCommunicationHelper.Instance.RegisteredAccountLogins[accountName.ToLower()][2];
        ...
        public void RegisterAccountLogin(string accountName, long sessionId, long lastCharacter, long port)
        {
            try
            {
                string lower = accountName.ToLower();

                if (ServerCommunicationHelper.Instance.RegisteredAccountLogins.ContainsKey(lower))
                    ServerCommunicationHelper.Instance.RegisteredAccountLogins.Remove(lower);

                ServerCommunicationHelper.Instance.RegisteredAccountLogins[lower]
                    = new long[3];
                ServerCommunicationHelper.Instance.RegisteredAccountLogins[lower][0] = sessionId;
                ServerCommunicationHelper.Instance.RegisteredAccountLogins[lower][1] = lastCharacter;
                ServerCommunicationHelper.Instance.RegisteredAccountLogins[lower][2] = port;
        ...
        public string RetrieveRegisteredWorldservers(int sessionId)
        {
            if (ServerCommunicationHelper.Instance.WorldserverGroups.Any())
            {
                ...
                foreach (WorldserverGroupDTO worldserverGroup in ServerCommunicationHelper.Instance.WorldserverGroups)
                {
                    ...
                    {
                        if (world.Endpoint.TcpPort == 4016)
                            continue;
                        //TODO account limit
