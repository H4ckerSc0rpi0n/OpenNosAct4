...
namespace OpenNos.GameObject
{
    public class MapMonster : MapMonsterDTO
    {
        ...
        private void Move()
        {
            ...
            {
                Character character = ServerManager.Instance.Sessions.FirstOrDefault(s => s?.Character != null && ((s.Character.Faction == 1 && Monster.NpcMonsterVNum != 679) || (s.Character.Faction == 2 && Monster.NpcMonsterVNum != 680)) && s.Character.Hp > 0 && !s.Character.InvisibleGm && !s.Character.Invisible && s.Character.MapInstance == MapInstance && Map.GetDistance(new MapCell { X = MapX, Y = MapY }, new MapCell { X = s.Character.PositionX, Y = s.Character.PositionY }) < Monster.NoticeRange)?.Character;
                if (character != null)
                {
        ...
        private void TargetHit2(ClientSession targetSession, NpcMonsterSkill npcMonsterSkill, int damage, int hitmode)
        {
            ...
            if (npcMonsterSkill != null && (npcMonsterSkill.Skill.Range > 0 || npcMonsterSkill.Skill.TargetRange > 0))
            {
                ...
                {
                    if ((characterInRange.Faction == 1 && MonsterVNum == 679)
                        || (characterInRange.Faction == 2 && MonsterVNum == 680))
                    {
                        continue;
                    }
