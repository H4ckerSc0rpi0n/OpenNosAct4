...
namespace OpenNos.GameObject
{
    public static class NRunHandler
    {
        ...
        public static void NRun(ClientSession Session, byte type, short runner, short npcid)
        {
            ...
            switch (runner)
            {
                ...
                case 5002:
                    if (npc != null)
                    {
                        SwitchToA1Server(Session);
                    }
                    break;
                case 5001:
                    if (npc != null)
                    {
                        if (Session.Character.Faction == 1
                            || Session.Character.Faction == 2)
                        {
                            SwitchToA4Server(Session, true);
                        }
                        else
                        {
                            Session.SendPacket($"msg 0 You need a faction to access Act 4");
                        }
                    }
                    break;
                case 5011:
        ...
        public static void SwitchToA4Server(ClientSession Session, bool bNPC)
        {
            OpenNos.WebApi.Reference.ServerCommunicationClient.Instance.HubProxy.Invoke("RegisterAccountLogin", Session.Account.Name, Session.Account.LastSession, Session.Character.Slot, Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["WorldPort"]));

            Session.SendPacket("mz " + System.Configuration.ConfigurationManager.AppSettings["IPADDRESS"] + " 4016 0");
            Session.SendPacket("it " + (bNPC ? "1" : "2"));

            System.Reactive.Linq.Observable.Timer(TimeSpan.FromMilliseconds(1000))
            .Subscribe(
            o =>
            {
                if (bNPC)
                {
                    Session.Character.MapId = (short)(Session.Character.Faction == 1 ? 130 : 131);
                    Session.Character.MapX = 13;
        ...
        public static void SwitchToA1Server(ClientSession Session)
        {
            OpenNos.WebApi.Reference.ServerCommunicationClient.Instance.HubProxy.Invoke("RegisterAccountLogin", Session.Account.Name, Session.Account.LastSession, Session.Character.Slot, -1);

            Session.SendPacket("mz " + System.Configuration.ConfigurationManager.AppSettings["IPADDRESS"] + " " + Session.LastPort.ToString() + " 0");
