...
namespace OpenNos.GameObject
{
    public class ClientSession
    {
        ...
        public ClientSession(INetworkClient client)
        {
            ...
            // absolutely new instantiated Client has no LastPort
            LastPort = -1;
        ...
        private void HandlePackets()
        {
            ...
            while (_receiveQueue.TryDequeue(out packetData))
            {
                ...
                // Workaround (crack the code, huahua)
                if (packetConcatenated.Contains("DAC"))
                {
                    packetConcatenated += (char)0xFF;
                    packetConcatenated += "0 0";
                }
                    ...
                    if (_encryptor.HasCustomParameter)
                    {
                        ...
                        if (_waitForPacketsAmount.HasValue)
                        {
                            ...
                            else
                            {
                                ...
                                TriggerHandler(header, queuedPackets, true); // this one is login im sure
